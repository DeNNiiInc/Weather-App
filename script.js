...
async function reverseGeocode(lat, lon) {
    try {
        const response = await fetch(`${GEOCODING_API}?latitude=${lat}&longitude=${lon}&count=1&format=json`);
...
async function fetchWeather(lat, lon) {
    const params = new URLSearchParams({
        latitude: lat,
        longitude: lon,
        current: 'temperature_2m,relative_humidity_2m,apparent_temperature,is_day,precipitation,weather_code,cloud_cover,pressure_msl,surface_pressure,wind_speed_10m,wind_direction_10m,wind_gusts_10m,uv_index,dew_point_2m,visibility',
        hourly: 'temperature_2m,relative_humidity_2m,precipitation_probability,precipitation,weather_code,wind_speed_10m,uv_index,is_day',
...
function updateCurrentWeather() {
    ...
    document.getElementById('cityName').textContent = 
        currentLocation.country ? `${currentLocation.name}, ${currentLocation.country}` : currentLocation.name;
    ...
    document.getElementById('dewPoint').textContent = `Dew Point: ${Math.round(current.dew_point_2m)}°`;
    ...
    document.getElementById('windDirectionText').textContent = getCardinalDirection(windDir);
    ...
    document.getElementById('visibilityText').textContent = getVisibilityDescription(visKm);
    ...
    if (airQualityData && airQualityData.current) {
        ...
        document.getElementById('aqiO3').textContent = o3 ? `${Math.round(o3)} μg/m³` : '--';
        document.getElementById('aqiPm25').textContent = pm25 ? `${Math.round(pm25)} μg/m³` : '--';
...
function createForecastCard(daily, index) {
    ...
    const rainProb = daily.precipitation_probability_max ? Math.round(daily.precipitation_probability_max[index]) : 0;
...
function updateHourlyForecast() {
    ...
    for (let i = currentIndex; i < currentIndex + 24 && i < hourly.time.length; i++) {
        ...
        item.innerHTML = `
            <div class="hourly-time">${i === currentIndex ? 'Now' : formatTime(time)}</div>
...
function renderSunGraph(canvasId, sunrise, sunset) {
    ...
    if (now >= viewStart && now <= viewEnd) {
        ...
        if (t >= 0 && t <= 1) {
            // Bezier height at t
            sunY = (Math.pow(1-t, 2) * h * 0.8) + (2 * (1-t) * t * h * 0.1) + (Math.pow(t, 2) * h * 0.8);
        } else {
...
function renderMoonPhase(containerId, phase) {
    ...
    let path = '';
    // Full Moon
    if (phase.index === 4) path = '<circle cx="32" cy="32" r="28" fill="#f0f0f0" />';
    // New Moon
    else if (phase.index === 0) path = '<circle cx="32" cy="32" r="28" fill="#333" stroke="#555" />';
    // Crescent/Quarter approximation
    else path = `<circle cx="32" cy="32" r="28" fill="#333" stroke="#555" /><text x="32" y="38" font-size="24" fill="white" text-anchor="middle" font-weight="bold">${['🌑','🌒','🌓','🌔','🌕','🌖','🌗','🌘'][phase.index]}</text>`;
...
function getWeatherInfo(code) {
    ...
    const weatherCodes = {
        0: { main: 'Clear', description: 'Clear sky', icon: (d) => `https://openweathermap.org/img/wn/${d ? '01d' : '01n'}@4x.png` },
        1: { main: 'Clear', description: 'Mainly clear', icon: (d) => `https://openweathermap.org/img/wn/${d ? '01d' : '01n'}@4x.png` },
        2: { main: 'Clouds', description: 'Partly cloudy', icon: (d) => `https://openweathermap.org/img/wn/${d ? '02d' : '02n'}@4x.png` },
        3: { main: 'Clouds', description: 'Overcast', icon: (d) => `https://openweathermap.org/img/wn/03d@4x.png` },
        45: { main: 'Fog', description: 'Foggy', icon: (d) => `https://openweathermap.org/img/wn/50d@4x.png` },
        48: { main: 'Fog', description: 'Rime fog', icon: (d) => `https://openweathermap.org/img/wn/50d@4x.png` },
        51: { main: 'Drizzle', description: 'Light drizzle', icon: (d) => `https://openweathermap.org/img/wn/09d@4x.png` },
        53: { main: 'Drizzle', description: 'Moderate drizzle', icon: (d) => `https://openweathermap.org/img/wn/09d@4x.png` },
        55: { main: 'Drizzle', description: 'Dense drizzle', icon: (d) => `https://openweathermap.org/img/wn/09d@4x.png` },
        61: { main: 'Rain', description: 'Slight rain', icon: (d) => `https://openweathermap.org/img/wn/10d@4x.png` },
        63: { main: 'Rain', description: 'Moderate rain', icon: (d) => `https://openweathermap.org/img/wn/10d@4x.png` },
        65: { main: 'Rain', description: 'Heavy rain', icon: (d) => `https://openweathermap.org/img/wn/10d@4x.png` },
        71: { main: 'Snow', description: 'Slight snow', icon: (d) => `https://openweathermap.org/img/wn/13d@4x.png` },
        73: { main: 'Snow', description: 'Moderate snow', icon: (d) => `https://openweathermap.org/img/wn/13d@4x.png` },
        75: { main: 'Snow', description: 'Heavy snow', icon: (d) => `https://openweathermap.org/img/wn/13d@4x.png` },
        80: { main: 'Rain', description: 'Rain showers', icon: (d) => `https://openweathermap.org/img/wn/09d@4x.png` },
        81: { main: 'Rain', description: 'Mod. showers', icon: (d) => `https://openweathermap.org/img/wn/09d@4x.png` },
        82: { main: 'Rain', description: 'Violent showers', icon: (d) => `https://openweathermap.org/img/wn/09d@4x.png` },
        95: { main: 'Thunderstorm', description: 'Thunderstorm', icon: (d) => `https://openweathermap.org/img/wn/11d@4x.png` },
        96: { main: 'Thunderstorm', description: 'T-storm & hail', icon: (d) => `https://openweathermap.org/img/wn/11d@4x.png` },
        99: { main: 'Thunderstorm', description: 'Heavy hail', icon: (d) => `https://openweathermap.org/img/wn/11d@4x.png` }
    };
